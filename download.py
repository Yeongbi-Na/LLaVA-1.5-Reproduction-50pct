...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download and extract LLaVA datasets")
    parser.add_argument(
        "--dataset", type=str, default="llava-v1.5-instruct",
        help="Dataset ID to download (e.g., llava-v1.5-instruct, llava-laion-cc-sbu-558k)"
    )
    parser.add_argument(
        "--root_dir", type=str, default="./data",
        help="Root directory where data will be saved"
    )
    args = parser.parse_args()

    # 실행
    download_extract(args.dataset, Path(args.root_dir))
    print(f"Dataset '{args.dataset}' downloaded and extracted at {os.path.join(args.root_dir, 'download', args.dataset)}")
